## Маркетинговые метрики в e-commerce

### Задача
Были проведены две рекламные кампании. На каждую  потратили 250 тысяч рублей. В результате первой кампании в приложении зарегистрировался 171 человек (список №1), а в результате второй - 236 человек(список №2).
Какая рекламная кампания более успешна?

#### Список №1 id пользователей
...
### Вычислим метрики CAC, ROI и средний чек для каждой рекламной компании


````sql
SELECT ads_campaign,
      round(250000.0/ count(distinct user_id), 2) as cac,
       round((sum (revenue)-250000)/250000.0*100, 2) as roi,
...
FROM (SELECT ads_campaign,
      ...
      FROM   (SELECT order_id,
               ...
               case when user_id in (8631, 8632, 8638, 8643, 8657, 8673, 8706, 8707, 8715, 8723, 8732,
                    ...
                    else 'no' end as ads_campaign
        FROM   user_actions
        WHERE  order_id not in (SELECT order_id
                                FROM   user_actions
                                WHERE  action = 'cancel_order')
                                
       GROUP BY ads_campaign, order_id,user_id) kk
    LEFT JOIN (SELECT order_id,
               ...
               FROM   (SELECT order_id,
                       ...
                       WHERE  order_id not in (SELECT order_id
                                               FROM   user_actions
                                               WHERE  action = 'cancel_order')) t5
                   LEFT JOIN products using (product_id)
               GROUP BY order_id) tt using (order_id)
     WHERE  ads_campaign != 'no'
     GROUP BY ads_campaign,user_id ) l
GROUP BY ads_campaign
ORDER BY 2 desc

````
**Результат:**
| ads_campaign | cac     | roi   | avg_check |
|--------------|---------|-------|-----------|
| Кампания № 1 | 1461,99 | 7,75  | 369,00    |
| Кампания № 2 | 1068,38 | -6,45 | 381,37    |


### Найдем Retention rate для 0, 1 и 7 дней
...
FROM   (SELECT time::date as dt,
        ...
        FROM   user_actions) h
WHERE  ads_campaign != 'no'
   and day_number in (0, 1, 7)
GROUP BY ads_campaign, start_date, day_number
ORDER BY 1, 3
````
**Результат:**
| ads_campaign/day_number |  0  |  1  |  7  |
|-------------------------|-----|-----|-----|
| Кампания № 1            | 1   | 0,42| 0,22|
| Кампания № 2            | 1   | 0,17| 0,09|

 ##

Затраты на привлечение одного покупателя ниже у рекламной кампании №2, при этом рентабельность первой кампании выше и, следовательно, она приносит больше денег. Хотя средний чек пользователей из обоих рекламных каналов не сильно отличается, зато  Retention более чем в два раза выше у первой группы. Это и приводит к тому, что пользователи из первой группы приносят нам больше денег. На основании этого делаем вывод, что более эффективной оказалась рекламная кампания №1 и в дальнейшем в этот канал привлечения имеет смысл вкладывать больше бюджета.
